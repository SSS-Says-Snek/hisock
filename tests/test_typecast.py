...
class TestServerTypeCast:
    ...
    def test_type_cast_str(self):
        _str = _type_cast(
            str, b"Why hello there, fello human!", self.__class__.func_name
        )
    ...
    def test_type_cast_int(self):
        _int = _type_cast(int, b"6969696969696969", self.__class__.func_name)
    ...
    def test_type_cast_invalid_cast(self):
        with pytest.raises(InvalidTypeCast):
            _type_cast(
                bytearray, b"haha this will not do anything", self.__class__.func_name
            )
